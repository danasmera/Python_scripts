...
def main():
  p = optparse.OptionParser(description='email client', prog='emailer', version='0.1a', usage='%prog -f env_from -t env_to -s subject -H smtp-host -F from -T to -r reply -R return-address, -d debug_level -p port_number, -D date, -e helo, -S tls, -a attach, -b body -c certificate -k privatekey')
  ...
  ###To do: use idioms - less if..else, make it readable; exception handling; class; multithreading (send single email through multiple smtp servers at the same time).
  ...
  if not options.attach:
    ...
    server.ehlo()
    if debuglevel: server.set_debuglevel(debuglevel)
    ...
    if options.reply: msg['Reply-to']=options.reply
    if options.Return: msg['Return-path']=options.Return
    ...
    server = smtplib.SMTP(smtpserver)
    server.ehlo()
    if options.tls: server.starttls()
    if debuglevel: server.set_debuglevel(debuglevel)
